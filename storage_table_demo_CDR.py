...
resourcegroup_name = 'rg-cdr'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'stgcdr'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
car.RowKey = '001'
car.make = 'Krumplghini'
car.model = 'Guardian CSH3'
car.year = '2013'
car.color = 'puece'
car.price = 2180
table_service.insert_entity('itemstable', car)
print('Created entry for Krumplghini...')
time.sleep(1)
...
car.RowKey = '002'
car.make = 'FleeYaht'
car.model = 'Monster SML2'
car.year = '1997'
car.color = 'chartreuse'
car.price = 8628
table_service.insert_entity('itemstable', car)
print('Created entry for FleeYaht...')
time.sleep(1)
...
car.price = 100025
table_service.insert_entity('itemstable', car)
print('Created entry for Mercydays...')
time.sleep(1)
...
print('Created entry for Folders...\n')
time.sleep(1)

coffee = Entity()
coffee.PartitionKey = 'coffeeShop'
coffee.RowKey = '006'
coffee.brand = 'Nextafay'
coffee.flavor = 'Peanut Butter'
coffee.cupSize = 'medium'
coffee.price = 10.00
table_service.insert_entity('itemstable', coffee)
print('Created entry for Nextafay...\n')
time.sleep(1)

coffee = Entity()
coffee.PartitionKey = 'coffeeShop'
coffee.RowKey = '007'
coffee.brand = 'Gevalier'
coffee.flavor = 'Sardine'
coffee.cupSize = 'small'
coffee.price = .50
table_service.insert_entity('itemstable', coffee)
print('Created entry for Gevalier...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'carInventory'", select='model,color, price')
for item in items:
    print('Model: ' + items.model)
	print('Color: ' + items.color)
    print('Price: ' + str(items.price) + '\n')

items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeeShop'", select='brand,flavor,price')
for item in items:
    print('Brand: ' + items.brand)
	print('Flavor:' + items.flavor)
    print('Price: ' + str(items.price) + '\n')
